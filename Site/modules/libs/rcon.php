...
class Rcon
{
	 ...
	 public function CommandRead($command)
    {
		...
		if($command == 'list'){
			$players = explode("online:", $packet_pack['body']);
			if(strlen($players[1]) > 4){
				$players[1] = preg_replace('/[\x00-\x1F\x7F]/u', '', $players[1]); 
				$players[1] = str_replace(',', '',  strtolower($players[1]));
