...
function check_reg($name, $email, $password1, $password2, $sec_code = 1, $sec_code_session = 1) {
	...
	if (!preg_match("#^[a-z0-9.-]+$#i", $name)) $stop .= 'В логине используются запрещённые символы';
...
if( $doaction == "validating" AND !$stopregistration AND !$_POST['submit_val'] ) {
	...
	$md5_regpassword = md5(md5($regpassword));
	...
	if( $reg_error != "" ) {
		...
		if( ($_REQUEST['step'] != 2) and $config['registration_type'] ) {
			...
			msgbox( $lang['all_info'], $lang['confirm_ok'] . "<br /><br /><a class='validation_to' href=\"" . $config['http_home_url'] . "index.php?do=register&doaction=validating&step=2&id=" . rawurlencode( $_REQUEST['id'] ) . "\">" . $lang['reg_next'] . "</a>" );
			...
			//print_r("INSERT INTO " . USERPREFIX . "_users (name, password, email, reg_date, lastdate, user_group, info, signature, favorites, xfields, logged_ip, hash, sashok_hash) VALUES ('{$name}', '{$regpassword}', '{$email}', '{$add_time}', '{$add_time}', '{$config['reg_group']}', '', '', '', '', '{$_IP}', '{$hash}', '{$md5_regpassword}')");
			//exit();
			
			$db->query( "INSERT INTO " . USERPREFIX . "_users (name, password, email, reg_date, lastdate, user_group, info, signature, favorites, xfields, logged_ip, hash, sashok_hash) VALUES ('{$name}', '{$regpassword}', '{$email}', '{$add_time}', '{$add_time}', '{$config['reg_group']}', '', '', '', '', '{$_IP}', '{$hash}', '{$md5_regpassword}')" );
