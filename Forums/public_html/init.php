...
{	
	...
	public static function defaultConstants()
	{
		...
		if ( isset( self::$cicConfig['redis-guest-cache'] ) and self::$cicConfig['redis-guest-cache'] == 'on' )
		{
			$outputCache = 'Redis';
			$guestTimeout = 900;
		}
		...
		return array(
			...
			// to change them.
				...
				'SUITE_UNIQUE_KEY'	=> ( isset( $_SERVER['IPS_CIC'] ) and preg_match( '/^\/var\/www\/html\/(.+?)$/i', __DIR__, $matches ) ) ? str_replace( '/', '', $matches[1] ) : mb_substr( md5( 'null4410' . '$Rev: 3023$'), 10, 10 ),
				'CACHEBUST_KEY'		=> mb_substr( md5( 'null4410' . '$Rev: 3023$'), 10, 10 ),
	...
	public static function licenseKey( $forceRefresh = FALSE )
	{
		$response = array(
			...
			'products'   => array(
				'forums' => 1, 'calendar' => 1, 'blog' => 1, 'gallery' => 1, 'downloads' => 1, 'cms' => 1, 'nexus' => 1, 'spam' => 1, 'copyright' => 1, 'chat' => 0
			),
			'support'    => "None"
		);

		$licenseData = array( 'fetched' => time(), 'data' => $response, 'refetched' => 1 );
