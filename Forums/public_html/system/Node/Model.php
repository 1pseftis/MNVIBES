...
abstract class _Model extends \IPS\Patterns\ActiveRecord
{
	...
	public static function titleFromIndexData( $indexData, $itemData, $containerData )
	{
		...
		{
			return \IPS\Member::loggedIn()->language()->addToStack( 'club_container_title', FALSE, array( 'sprintf' => array( $containerData['_club']['name'], \IPS\Member::loggedIn()->language()->addToStack( static::$titleLangPrefix . $indexData['index_container_id'], 'NULL', array( 'escape' => true ) ) ) ) );
		}
		else
		{
			return \IPS\Member::loggedIn()->language()->addToStack( static::$titleLangPrefix . $indexData['index_container_id'], 'NULL', array( 'escape' => true ) );
		}
	}
	...
	public function metaDescription()
	{
		if ( static::$titleLangPrefix and static::$descriptionLangSuffix )
		{
			return \IPS\Member::loggedIn()->language()->addToStack( static::$titleLangPrefix . $this->id . static::$descriptionLangSuffix, FALSE, array( 'striptags' => TRUE, 'escape' => TRUE ) );
		}
		return NULL;
	}
	...
	public function can( $permission, $member=NULL, $considerPostBeforeRegistering = TRUE )
	{
		...
		if( $member instanceof \IPS\Member\Group )
		{
			...
			return \in_array( $groupId, $relevantPermissionArray );
		}
		else
		{
			...
			{
				return \in_array( \IPS\Settings::i()->member_group, $relevantPermissionArray );
			}
			else
			{
