...
class _Theme extends \IPS\Node\Model
{
	...
	public function get_logo_sharer()
	{
		...
		if( \count( $shareLogos ) )
		{
			try
			{
				return \IPS\File::get( 'core_Icons', $shareLogos[0] )->url->setScheme( ( \IPS\Request::i()->isSecure() ) ? 'https' : 'http' );
			}
			catch( \Exception $e )
			{
				return '';
	...
	{		
		if( \IPS\Settings::i()->icons_favicon )
		{
			try
			{
				return \IPS\File::get( 'core_Icons', \IPS\Settings::i()->icons_favicon )->url;
			}
			catch( \Exception $e )
			{
				return '';
	...
	public function website( $munged = FALSE )
	{
		...
		{
			return ( $munged ) ? \IPS\Http\Url::createFromString( $this->_data['author_url'] )->makeSafeForAcp() : \IPS\Http\Url::createFromString( $this->_data['author_url'] );
		}
		return NULL;
	}
...
EOF;
	...
	{{if !\$licenseData = \IPS\IPS::licenseKey() or !isset(\$licenseData['products']['copyright']) or !\$licenseData['products']['copyright']}}<a rel='nofollow' title='Invision Community' href='https://www.invisioncommunity.com/'>Powered by Invision Community</a>{{endif}}
</p>";
		}
