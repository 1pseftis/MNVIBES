...
abstract class _Item extends \IPS\Content
{
	...
	public static function createFromForm( $values, \IPS\Node\Model $container = NULL, $sendNotification = TRUE )
	{
		...
		if ( isset( static::$commentClass ) and static::$firstCommentRequired )
		{
			...
			$parameters = array_merge( array( 'newContentItem-' . static::$application . '/' . static::$module  . '-' . ( $container ? $container->_id : 0 ) ), $comment->attachmentIds() );
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		{
			$return['title'] = new \IPS\Helpers\Form\Text( static::$formLangPrefix . 'title', $item ? $item->mapped('title') : ( isset( \IPS\Request::i()->title ) ? \IPS\Request::i()->title : NULL ), TRUE, array( 'maxLength' => \IPS\Settings::i()->max_title_length ?: 255, 'bypassProfanity' => \IPS\Helpers\Form\Text::BYPASS_PROFANITY_SWAP ) );
		}
	...
	public static function tagsFormField( $item, $container, $minimized = FALSE )
	{
		...
		$options = array( 'autocomplete' => array( 'unique' => TRUE, 'source' => $source, 'resultItemTemplate' => 'core.autocomplete.tagsResultItem', 'freeChoice' => \IPS\Settings::i()->tags_open_system ? TRUE : FALSE ) );
	...
	public function delete()
	{
		...
		if ( isset( static::$commentClass ) )
		{
			...
			\IPS\Db::i()->delete( 'core_reputation_index', array( "rep_class=? AND " . \IPS\Db::i()->in( 'type_id', $commentIds ), $commentClass ) );
		...
		if ( isset( static::$reviewClass ) )
		{
			...
			\IPS\Db::i()->delete( 'core_reputation_index', array( "rep_class=? AND " . \IPS\Db::i()->in( 'type_id', $reviewIds ), $reviewClass ) );
	...
	public function changeAuthor( \IPS\Member $newAuthor, $log=TRUE )
	{
		...
		if ( static::incrementPostCount( $this->containerWrapper() ) AND ( $oldAuthor->member_id OR $this->hidden() === 0 ) )
		{
	...
	protected function _comments( $class, $limit, $offset=NULL, $order='date DESC', $member=NULL, $includeHidden=NULL, $cutoff=NULL, $canViewWarn=NULL, $extraWhereClause=NULL, $includeDeleted=FALSE )
	{
		...
		if( $limit == 1 AND !$offset AND mb_strtolower( $order ) == mb_strtolower( $class::$databasePrefix . $class::$databaseColumnMap['date'] . ' desc' ) )
		{
			try
			{
				$id = $class::db()->select( 'MAX(' . $class::$databasePrefix . $class::$databaseColumnId . ')', $class::$databaseTable, $where )->first();
				
				$where  = array( array( $class::$databaseTable . '.' . $class::$databasePrefix . $class::$databaseColumnId . '=?', (int) $id ) );
				$offset = 0;
				$order  = NULL;
			}
			catch( \UnderflowException $e ){}
		...
		elseif( $limit == 1 AND !$offset AND mb_strtolower( $order ) == mb_strtolower( $class::$databasePrefix . $class::$databaseColumnMap['date'] . ' asc' ) )
		{
			try
			{
				$id = $class::db()->select( 'MIN(' . $class::$databaseTable . '.' . $class::$databasePrefix . $class::$databaseColumnId . ')', $class::$databaseTable, $where )->first();
	...
	public function processCommentForm( $values )
	{
		...
		$parameters = array_merge( array( 'reply-' . static::$application . '/' . static::$module  . '-' . $this->$idColumn ), $comment->attachmentIds() );
		\IPS\File::claimAttachments( ...$parameters );
	...
	public function canView( $member=NULL )
	{
		...
		if ( $this instanceof \IPS\Content\Hideable and $this->hidden() )
		{
			...
			elseif ( $this->hidden() === 1 and $this->author()->member_id == $member->member_id )
			{
				// OK
			}
