...
EOF;
	...
	{		
		...
		$classes = array();
		$jsonFile = \IPS\ROOT_PATH . "/applications/{$this->directory}/data/extensions.json";
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		/* Upgrade */
		if( !$this->protected AND !\IPS\DEMO_MODE )
		{
	 ...
	 *
	 * @param bool $munged	Munge the URL
	 *
	 * @return \IPS\Http\Url|null
	 */
	public function website( $munged = FALSE )
	{
		if ( $this->_data['website'] )
		{
			return ( $munged ) ? \IPS\Http\Url::createFromString( $this->_data['website'] )->makeSafeForAcp() : \IPS\Http\Url::createFromString( $this->_data['website'] );
		}
		return NULL;
	}
	...
	public function canAccess( $memberOrGroup=NULL )
	{
		...
		if( \IPS\Dispatcher::hasInstance() AND \IPS\Dispatcher::i()->controllerLocation === 'admin' AND \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'applications', 'app_manage' ) )
		{
			return TRUE;
		}
	...
	{		
		...
		/* Remove the files and folders, if possible (if not IN_DEV and not in DEMO_MODE) */
		if ( !\IPS\IN_DEV AND !\IPS\DEMO_MODE AND file_exists( \IPS\ROOT_PATH . '/applications/' . $this->directory ) )
		{
			try
			{
